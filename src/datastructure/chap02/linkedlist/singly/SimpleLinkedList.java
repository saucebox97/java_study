...
public class SimpleLinkedList {
    ...
    public void insert(int data) {
        ...
        // 바로 이전 노드를 가져오기
        Node prev = null; // 처음에 이전노드가없으니 null
        // 삽입 위치 탐색
        // 빈 리스트가 아니고, 삽입할 데이터가 현재노드의 데이터보다 작아질때까지지
        while (current != null && data > current.getData()) { // 반복문을 통해서 내가 들어갈곳을 찾는다
            // 현재 노드를 이전노드에 백업
            prev = current;
            // 다음 노드를 현재 노드로 재설정
    ...
    public Node remove(int data) {
        ...
        if (prev == null && current != null) {
            ...
            Node secondNode = current.getNextNode(); //header.getFirstNode().getNextNode();
            header.setFirstNode(secondNode);
        }
        // 삭제 대상이 중간 어딘가인 경우 마지막도포함
        else if (prev != null && current != null) { // && and연산자 둘다 맞아야됌
