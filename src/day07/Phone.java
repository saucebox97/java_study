...
public class Phone {
    ...
    void sendMessage(Phone targetPhone, String msg) {
        // 상대방의 메시지함이 가득차있는가?
        if (targetPhone.receiveMessages.length > targetPhone.massageCount) {
            // 상대방 메시지함에 메시지 넣기 targetPhone(상대방)
            targetPhone.receiveMessages[targetPhone.massageCount++] = msg + "by" + model;
        } else { // 가득 찬 경우
    ...
    void delMessages() {
        receiveMessages[massageCount--] = null;
    }
